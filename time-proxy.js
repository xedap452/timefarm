...
const { getQueryId } = require('./dancayairdrop');
...
class Timefarm {
    ...
    loadToken(queryId) {
        ...
        return tokens[queryId] || null;
    }

	saveToken(accountNumber, token) {
		const tokens = JSON.parse(fs.readFileSync('token.json', 'utf8'));
		tokens[accountNumber] = token;
	...
	async getBalance(upgradeWatch, proxy, accountNumber, apiId, apiHash, sessionFilePath) {
		...
		while (true) {
			try {
				...
				try {
					const queryId = await getQueryId(apiId, apiHash, sessionFilePath);
					if (!queryId) {
						console.error('queryId không hợp lệ hoặc không được lấy.');
						await this.countdown(60);
						continue;
					}

					const loginResult = await this.login(queryId, proxy);
					if (loginResult && loginResult.token) {
						const { token: newToken } = loginResult;
						this.saveToken(accountNumber, newToken);
					...
					console.error('Lỗi khi lấy query_id hoặc đăng nhập:', loginError);
					await this.countdown(60);
					continue;
				}
			}
	...
	async main() {
		...
		readline.question('Có nâng cấp đồng hồ không? (y/n) ', async (answer) => {
			...
			const tokens = fs.existsSync('token.json') ? JSON.parse(fs.readFileSync('token.json', 'utf8')) : {};

			while (true) {
				const listCountdown = [];
				const start = Math.floor(Date.now() / 1000);
				for (let i = 0; i < datas.length; i++) {
					const [apiIdString, apiHash, sessionFilePath] = datas[i].split('|');
					const apiId = parseInt(apiIdString, 10);
					const proxyUrl = proxies[i];
					const proxy = new HttpsProxyAgent(proxyUrl);

					const accountNumber = (i + 1).toString();
					let token = tokens[accountNumber] || null;

					console.log(`========== Tài khoản ${accountNumber} | IP: ${await this.checkProxyIP(proxy)} ==========`);
					...
					if (!token) {
						...
						try {
							...
							console.error('Lỗi khi lấy query_id hoặc đăng nhập:', error);
							continue;
						}
					} else {
					...
					const result = await this.getBalance(upgradeWatch, proxy, accountNumber, apiId, apiHash, sessionFilePath);
